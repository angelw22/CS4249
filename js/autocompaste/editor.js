  ...
  function Editor (textarea, engine) {
    ...
    this._fillCompletionList = function _fillCompletionList (suggestions) {
      ...
      for (var i = 0; i < suggestions.length; i++) {
        ...
        sentence = sentence.substring(suggestions[i].start, suggestions[i].start) + 
    ...
    this._completeWithFocusedItem = function _completeWithFocusedItem () {
      ...
      console.log(sentence);
      var bounds = {
        "start" : privates.textarea.value.lastIndexOf(sentence),
        "end" : privates.textarea.value.length
      };
      console.log("Bounds: ");
      console.log(bounds);
      if (bounds == undefined) {
      ...
      var new_value = textarea_value.substring(0, bounds.start) +
                        suggestion.sentence.substring(suggestion.start) + ' ' +
    ...
    privates.clist_prepend_start = -1;
    privates.clist_prepend_stack = [];
    ...
    privates.textarea.addEventListener('input', function (input_event) {
      ...
      console.log("Curr sentence: " + sentence);
    ...
    privates.textarea.addEventListener('keydown', function (keydown_event) {
      if (privates.clist_shown) {
        ...
        if (keydown_event.keyCode == 39) {
          ...
          if (privates.clist_next != undefined) {
            var textarea = privates.textarea;
            var caret_position = textarea.selectionStart;
            ...
            if (privates.clist_prepend_start > -1) {
              var textarea = privates.textarea;
              textarea.value = textarea.value + " " + privates.clist_prepend_stack.pop();
              privates.clist_prepend_start--;
              keydown_event.preventDefault();
            }
          }
        ...
        if (keydown_event.keyCode == 37) {
          if (privates.clist_append_stack.length > 0) {
            ...
            privates.clist_append_start = -1;
            var textarea = privates.textarea;
            var x = textarea.value.lastIndexOf(" ");
            if (textarea.value.substring(x + 1).trim() != "") {
              privates.clist_prepend_stack.push(textarea.value.substring(x + 1));
              privates.clist_prepend_start++;
              console.log("Pushing " + textarea.value.substring(x + 1));
            }
            textarea.value = textarea.value.substring(0, x);
            keydown_event.preventDefault();
